...
export class AuthController {
  ...
  async generateToken(@Headers('x-admin-secret') secret: string) {
    ...
    const isBetaMode =
      this.configService.get<string>('BETA_TESTING_MODE') === 'true';

    if (isBetaMode) {
      if (!secret) {
        throw new UnauthorizedException(
          'Please provide any string as a beta-secret header',
        );
      }
      console.log(`Beta token generated by tester using key: ${secret}`);
